...
用户的硬件项目照片基本是 iPhone HEIC（2026-06 实战管线）：

1. **ffmpeg 转 HEIC 不能直接 `-vf scale`**——iPhone HEIC 是 tiled 格式，解码自动走 complex filtergraph，跟 `-vf` 冲突直接报错。两步走：先 `ffmpeg -i in.HEIC -q:v 4 full.jpg` 全尺寸转出，再从 full.jpg 缩放出 web 尺寸（长边 1400-1800、`-q:v 3`，单张 ~200KB）
2. **Windows git-bash 的 `/tmp` Read 工具看不见**——`cygpath -w /tmp/xxx` 换成真实 Windows 路径再 Read 预览
3. **批量照片按叙事节点挑子集**：十几张里挑 5-7 张，每张对应一个章节拍点；近似构图只留一张。**有手入镜的先放大检查**（用户曾因"手脱皮有碍瞻观"打回两张，最终换成无手的下单后台渲染截图）
4. **手机截图先裁**：订单 / 聊天截图用 `crop=W:H:x:y` 裁掉状态栏和无关条目，只留有效区域；竖长全屏截图直接入文太占版面
5. **PDF 原理图 → 配图**：`pip install pymupdf`，按目标宽度算 zoom 渲成 PNG（线条图用 PNG 不糊，2400px 宽 ~260KB）

## 图注不可见：故事进正文

本主题**没有 figcaption 渲染**——markdown 图片的 alt 只是无障碍 / SEO 文本，**读者看不见**。所以：

- 故事性细节（"加热台有点溢价但颜值高"）必须织进正文，塞 alt 等于丢失
- alt 写客观描述（拍了什么、关键信息点），别藏包袱
- 想要图注式幽默，用图片紧前 / 紧后的正文句子承接
...
5. ❌ 加深色/突兀的标签条（喧宾夺主）
6. ❌ 把故事细节写进 alt 当图注（figcaption 不渲染，读者看不见）
