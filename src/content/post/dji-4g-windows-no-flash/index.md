...
description: "实测大疆一代 4G 模块：不刷机、不改模块配置，只为两个功能接口手动选择签名驱动，即可在 Windows 查询状态、收发短信和通过中国电信 4G 上网。"
category: "硬件折腾"
publishDate: "2026-07-20T15:40:00+08:00"
updatedDate: "2026-07-20T17:12:00+08:00"
...
- **VID/PID**：每个 USB 设备都有的一对「身份证号」。VID 表示厂商，PID 表示产品型号；Windows 会据此决定能否自动找到对应驱动。
- **MI_02、MI_04**：这块模块插到电脑后，实际上同时提供了好几个功能；`MI` 后的数字只是其中一个功能接口的编号，不是模块型号。
- **AT 指令**：向 4G 模块发送的短文本命令，用于查询 SIM 卡、信号、运营商和短信等状态。发送最简单的 `AT`，模块回复 `OK`，就说明串口能通信。
- **WWAN**：Windows 对「蜂窝网络网卡」的称呼，也就是让电脑通过 SIM 卡使用 4G 上网的那张网卡。

这次实测没有向模块写入或改动任何配置。做的事情只有两件：

1. 为用于发送 AT 指令的 `MI_02` 选择串口驱动；
2. 为用于 4G 上网的 `MI_04` 选择蜂窝网卡驱动。

最后的结果：

| 项目        | 实测结果                                |
| ----------- | --------------------------------------- |
| 控制串口    | 正常，Windows 分配为 `COM3`             |
...
其中 `VID_2CA3` 是厂商编号，`PID_4006` 是产品编号。普通人不需要记住它们；后面的安装步骤只用它来确认自己选中了模块中正确的功能接口。
...
它本质上是移远 EG25-G 系列方案，只是原厂状态使用 DJI 的 USB 身份证号 `2CA3:4006`，而非常见的移远编号。硬件和协议能力都在，Windows 自动匹配不到的只是这张「身份证」。

把它看成一栋房子会更好理解：模块只插了一根 USB 线，但里面有五个不同用途的「房间」。我们只需要进入第 2 个和第 4 个：
...
> [!NOTE]
> `MI_02` 不是型号，而是 USB 复合设备的第 2 号接口。无论 Windows 给它分配成 COM3、COM4 还是别的编号，能发送 `AT` 并收到 `OK` 才算找对。

## 「不刷机」到底省略了什么

很多教程所说的「刷机」，实际是在模块里把 USB 身份证号从：
...
改完后 Linux 能自动找到常见驱动，使用起来确实方便；但这个改动会写入模块，拔线或卸载 Windows 驱动都不会还原。

本文完全不做这一步。代价是 Windows 不会自动匹配驱动，需要在设备管理器里手动指定。好处也很直接：模块保持原厂身份，整个过程可以通过卸载 Windows 设备驱动撤销。

真正替换模块系统软件的「刷固件」风险更高，还可能影响通信校准和设备身份。为了查询状态、收发短信和 4G 上网，没有必要碰它。
...
这一步的目标是让 Windows 给模块分配一个 COM 端口，之后就能用普通串口工具或 PowerShell 向它发送查询命令。
...
下面这段 PowerShell 只执行查询，不修改模块配置。将 `COM3` 换成设备管理器中的实际端口；它相当于依次问模块「能通信吗、SIM 能用吗、信号和运营商是什么」。
...
QDC507 默认通常使用 PDU 模式，也就是把短信保存成机器可读的编码。中文长短信会拆成多个分片；读取时需要按顺序合并并解码，短信内容本身没有丢失。
...
这一步完成后，模块会像电脑内置的手机网络功能一样，出现在 Windows 的网络连接中。
...
APN 可以理解成运营商给手机或网卡配置的「上网入口名称」。普通手机通常自动填写；这块模块在 Windows 中可能需要手动填一次。
...
同一运营商换卡时，APN 通常不变；换运营商时一般要换 APN。Windows 会把这份配置和具体 SIM 卡关联，因此即使同运营商换卡，旧配置也可能不再匹配。
...
这次最有价值的结论不是「QDC507 能上网」——Linux 社区早就证明了。真正补上的拼图是：**模块的 USB 身份证号不同，并不妨碍它在 Windows 下查询状态、收发短信和使用 4G 数据；它只让 Windows 无法自动匹配驱动。**
