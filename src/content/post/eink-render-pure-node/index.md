...
上面那个「我自己编的 FreeType-WASM」，当时是为这块屏剪出来的窄版——单 MONO、手挑了几个函数、塞在 home-pi 仓库的 `vendor/` 里。能用，但只够我自己用。

后来把它做对了，单拎成一个独立公开库 [zkl2333/freetype-wasm](https://github.com/zkl2333/freetype-wasm)：完整 FreeType 公共 API，内存照样能涨，tag 跟上游 FreeType 一一对应（不自造版本号），定时任务自动追新版、构建验证过了才打 tag，jsdelivr 和 `npm i github:` 直接能拿。

pi 这边改成吃它的钉版产物（入库、不联网——Pi 连不上 github.com）。换库前对了一遍：新库的 FreeType 2.14.3 跟原来自编的 2.13.3，同字体同字号 1144 个样本，advance、几何、MONO 位图、竖直度量**逐字节一致**，零回归，真机上屏也验过。

说白了就是：当时图快剪了个窄版凑合，现在把它做对、单拎出来，谁都能用。这一步也是一个会话里跟 Claude 一口气弄完的。
