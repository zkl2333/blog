...
```mermaid
flowchart TD
    A["34 源 RSS + DeepSeek 官网监控"] -->|每小时抓| B["① URL 去重<br/>归一化、砍 utm / ref"]
    B --> C["② 标题 SimHash<br/>64 位指纹，差 ≤3 位算重复"]
    C --> D["③ 内容指纹<br/>网页源整页哈希，没变跳过"]
    D --> E["进 news_pool · 72h 滚动池"]
```
...
```mermaid
flowchart TD
    A["news_pool · 72h 内的条目"] --> B["标题归一化<br/>去停用词、中文 2-gram 切词"]
    B --> C["倒排索引挑出有共同词的候选对"]
    C --> D["算 Jaccard 相似度<br/>共用词占比 ≥ 0.55 算同一事件"]
    D --> E["并查集传递合并<br/>A 像 B、B 像 C 串成一摊"]
    E --> F["按源权威度选代表<br/>其余折叠进『也有报道自』"]
    F --> G["events_72h"]
```
...
```mermaid
flowchart TD
    A["最近 4 小时的 news_pool"] --> B["按关键词、正则聚成话题"]
    B --> C{够爆炸级吗}
    C -->|"≥2 高可靠源 / 官方旗舰发布<br/>≥5 源中英交叉 / 社区 ≥20 条"| D["立即推送<br/>同主题 24h 内不重推"]
    C -->|普通热点| E["静默存进 teaser<br/>等次日早报一起收"]
```
...
```mermaid
flowchart TD
    A["events_72h · 去重合并好的清单"] --> B["每早 8:30 大模型读取<br/>禁止联网"]
    B --> C["按口味筛 8-12 条<br/>动态分栏目、配链接"]
    C --> D["微信 → 邮件"]
```
