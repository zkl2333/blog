---
title: "AI 让我膨胀之我要画板"
description: "被 AI 惯出来的胆子：第一次自己画板，树莓派 Zero 外形的 ESP32-C3。打样回来无限重启，修了三回——第一回抓了共犯当主犯，第二回发现压根没烧过固件，第三回才是真欠压。最后一颗雷，Claude 排障排到了自己头上。"
pinned: true
...
「毕业」那句写早了。后来还拖着两个尾巴，才算收干净。

一是电源。我把那颗超规格的 1k 充电电阻换回原设计的 10k，板子立刻又无限重启，串口抓到 `E BOD`，欠压。这才搞懂：这板是 charger-only，充电电阻同时卡着「USB 到系统」的供电上限，10k 只放行 120mA，喂不饱 ESP32 的 345mA 峰值——原设计这颗就选错了。再叠上一处松掉的电池接触，重启得格外邪门。接好电池、换回大电流的电阻，才稳。

二是温度计。SHT40 我焊上了，i2c 却死活扫不到（`Found no devices`），可电压量着全对，芯片明明焊得好好的。折腾半天，是固件的 I²C 引脚配错了：我照着设计文档填了 IO0/IO1，但那是下一版（v2）的规划，这块 v1 实物的 SDA/SCL 接的是 IO8/IO9。对着没接传感器的脚扫，当然空。

```yaml
i2c:
  sda: GPIO8   # v1 实物；IO0/IO1 是文档里 v2 的规划，被我抄串了
  scl: GPIO9
```

```
Found device at address 0x46
Temperature >> 33.08 °C
Humidity   >> 46.33 %
```

（33°C 偏高，是板子自己在发热，传感器贴在正充电的板上。）温湿度、电压、WiFi 全往 Home Assistant 报，到这一刻，它才算一支真正的温度计。

又是「调用错误」。这次我栽在自己写的文档上——拿 v2 的图，配了 v1 的板。
